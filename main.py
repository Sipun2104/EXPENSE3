...
def register():
    if request.method == "POST":
        ...
        try:
            cur.execute(
                "INSERT INTO users (username, email, password_hash, daily_limit) VALUES (?,?,?,?)",
                (username, email, generate_password_hash(password), daily_limit)
            )
            conn.commit()
...
def forgot_password():
    if request.method == "POST":
        ...
        if user:
            ...
            conn.execute("UPDATE users SET reset_token=?, reset_expiry=? WHERE id=?", (reset_token, expiry, user["id"]))
            conn.commit()
...
def reset_password():
    ...
    conn = get_db()
    user = conn.execute("SELECT id, reset_expiry FROM users WHERE reset_token=?", (token,)).fetchone()
    conn.close()
    ...
    if request.method == "POST":
        ...
        conn = get_db()
        conn.execute("UPDATE users SET password_hash=?, reset_token=NULL, reset_expiry=NULL WHERE id=?", (generate_password_hash(password), user["id"]))
        conn.commit()
        conn.close()
